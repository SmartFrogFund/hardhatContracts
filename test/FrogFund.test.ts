...
describe( "FrogFund", function () {
    ...
    beforeEach( async function () {
        ...
        const res1 = await token.transfer( addr1.address, ethers.parseUnits( "1000", 18 ) );
        const res2 = await token.transfer( addr2.address, ethers.parseUnits( "1000", 18 ) );
  ...
  describe("supportProject", function () {
    it("Should allow users to support a project", async function () {
         time.advanceBlock();
      const goalAmount = ethers.parseUnits("500", 18);
      const deadline = (await time.latest()).add(time.duration.days(7)).toNumber();

      await frogFund.connect(addr1).createProject(goalAmount, deadline);

      const supportAmount = ethers.parseUnits("100", 18);
      console.log(`授权 token 转移...`);
      await token.connect(addr2).approve(await frogFund.getAddress(), supportAmount);
