...
contract FrogFund is Ownable {
    ...
    event ProjectCreated(uint256 indexed projectId, address indexed creator, uint256 goalAmount, uint256 deadline);
    event ProjectFunded(uint256 indexed projectId, address indexed supporter, uint256 amount);
    event ProgressUpdated(uint256 indexed projectId, uint256 progress, string details);
    event FundsDistributed(uint256 indexed projectId, uint256 amount);
    event RefundIssued(uint256 indexed projectId, address indexed supporter, uint256 amount);
    event ProgressReviewed(uint256 indexed projectId, string comment, bool approved);
    ...
    function updateProgress(uint256 _projectId, uint256 _progress, string calldata _details) external {
        Project storage project = projects[_projectId];
        require(msg.sender == project.creator, "Only project creator can update progress");
        require(_progress == 30 || _progress == 50 || _progress == 70 || _progress == 100, "Invalid progress value");
        require(_progress > project.currentProgress, "Progress must be greater than current progress");
    ...
    function reviewProgress(uint256 _projectId, string calldata _comment, bool _approved) external onlyOwner {
        Project storage project = projects[_projectId];
        ...
        if (_approved) {
            uint256 amountToDistribute = (project.goalAmount * project.currentProgress) / 100;
            require(token.transfer(project.creator, amountToDistribute), "Token transfer failed");
    ...
    function distributeFunds(uint256 _projectId) external {
        ...
        if (project.currentAmount >= project.goalAmount) {
            require(token.transfer(project.creator, project.currentAmount), "Token transfer failed");
            ...
            for (uint256 i = 0; i < projectCount; i++) {
                ...
                if (contribution > 0) {
                    ...
                    require(token.transfer(msg.sender, contribution), "Refund transfer failed");
    ...
    function updateProject(uint256 _projectId, uint256 _newGoalAmount, uint256 _newDeadline) external {
        Project storage project = projects[_projectId];
        require(msg.sender == project.creator, "Only project creator can update");
    ...
    function getProjectDetails(uint256 _projectId) external view returns (address, uint256, uint256, uint256, bool, uint256) {
        Project storage project = projects[_projectId];
        return (project.creator, project.goalAmount, project.currentAmount, project.deadline, project.completed, project.currentProgress);
    }
}
